...
local Dashboard = TSM.MainUI:NewPackage("Dashboard") ---@type AddonPackage
...
local ROUNDING_VALUE = ClientInfo.IsRetail() and COPPER_PER_GOLD or 1
...
function Dashboard.OnInitialize(settingsDB)
...
function private.GraphFormatY(value, suggestedStep, isTooltip)
	if isTooltip then
		return Money.ToStringForUIShort(value)
	end
	if not ClientInfo.IsRetail() and value < COPPER_PER_GOLD * 1000 then
		-- "###g"
		return floor(value / COPPER_PER_GOLD)..MoneyFormatter.GetGoldText()
	elseif not ClientInfo.IsRetail() and value < COPPER_PER_GOLD * 1000 * 10 then
		-- "#.##Kg"
		return format("%.2f", value / (COPPER_PER_GOLD * 1000)).."k"..MoneyFormatter.GetGoldText()
	elseif value < COPPER_PER_GOLD * 1000 * 1000 then
		-- "###Kg"
		return floor(value / (COPPER_PER_GOLD * 1000)).."k"..MoneyFormatter.GetGoldText()
	elseif value < COPPER_PER_GOLD * 1000 * 1000 * 10 then
		-- "#.##Mg"
		return format("%.2f", value / (COPPER_PER_GOLD * 1000 * 1000)).."M"..MoneyFormatter.GetGoldText()
	else
		-- "###Mg"
		return floor(value / (COPPER_PER_GOLD * 1000 * 1000)).."M"..MoneyFormatter.GetGoldText()
	end
end
...
function private.PopulateDetails(contentFrame)
	...
	contentFrame:GetElement("summary.range.low.value")
		:SetText(rangeLow and Money.ToStringForUIShort(rangeLow) or "-")
	contentFrame:GetElement("summary.range.high.value")
		:SetText(rangeHigh and Money.ToStringForUIShort(rangeHigh) or "-")

	contentFrame:GetElement("summary.daily.sales.value")
		:SetText(Math.Round(saleTotalQuantity / numDays))
	contentFrame:GetElement("summary.daily.purchases.value")
		:SetText(Math.Round(buyTotalQuantity / numDays))

	contentFrame:GetElement("summary.top.sale.value")
		:SetText(Money.ToStringForUIShort(Math.Round(saleTopValue, ROUNDING_VALUE)))
	contentFrame:GetElement("summary.top.expense.value")
		:SetText(Money.ToStringForUIShort(Math.Round(buyTopValue, ROUNDING_VALUE)))

	contentFrame:GetElement("details.salesTotal.amount")
		:SetText(Money.ToStringForUI(saleTotal))
	contentFrame:GetElement("details.salesAvg.amount")
		:SetText(Money.ToStringForUI(salePerDay))
	...
	contentFrame:GetElement("details.expensesTotal.amount")
		:SetText(Money.ToStringForUI(buyTotal))
	contentFrame:GetElement("details.expensesAvg.amount")
		:SetText(Money.ToStringForUI(buyPerDay))
	...
	contentFrame:GetElement("details.profitTotal.amount")
		:SetText(Money.ToStringForUI(profitTotal, profitTotal < 0 and Theme.GetColor("FEEDBACK_RED"):GetTextColorPrefix() or nil))
	contentFrame:GetElement("details.profitAvg.amount")
		:SetText(Money.ToStringForUI(profitPerDay, profitPerDay < 0 and Theme.GetColor("FEEDBACK_RED"):GetTextColorPrefix() or nil))
