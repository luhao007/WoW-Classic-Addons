...
local SyntaxHelpers = LibTSMService:Init("UI.SyntaxHelpers") ---@class UI.SyntaxHelpers
local Theme = LibTSMService:Include("UI.Theme")
local MoneyFormatter = LibTSMService:From("LibTSMUtil"):IncludeClassType("MoneyFormatter")
...
function private.InsertColoredToken(tokenType, str)
	...
	elseif tokenType == CustomString.TOKEN_TYPE.MONEY then
		...
		if symbol == "g" then
			coloredSymbol = MoneyFormatter.GetGoldText()
		elseif symbol == "s" then
			coloredSymbol = MoneyFormatter.GetSilverText()
		elseif symbol == "c" then
			coloredSymbol = MoneyFormatter.GetCopperText()
		else
	...
	elseif tokenType == CustomString.TOKEN_TYPE.IDENTIFIER or tokenType == CustomString.TOKEN_TYPE.FUNCTION then
		...
		if CustomString.IsReservedWord(lowerStr) and lowerStr ~= "convert" then
			color = FUNCTION_COLOR
		elseif CustomString.IsSourceRegistered(lowerStr) or lowerStr == "convert" then
			color = SOURCE_COLOR
		elseif CustomString.IsCustomSourceRegistered(lowerStr) then
			color = CUSTOM_SOURCE_COLOR
		elseif lowerStr == "baseitem" or strmatch(str, "^[ip]:%d+") then
			color = CUSTOM_SOURCE_COLOR
		else
			color = UNKNOWN_COLOR
		end
