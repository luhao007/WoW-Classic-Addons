...
local FSMObject = LibTSMUtil:DefineClassType("FSMObject")
...
---@param name string The name of the FSM (for debugging)
---@return FSMObject
function FSMObject.__static.New(name)
...
function FSMObject.__private:__init(name)
	self._name = name
...
function FSMObject.__private:_Transition(eventResult)
