...
local private = {
	objectPool = ObjectPool.New("DATABASE_QUERY_CLAUSES", DatabaseQueryClause, 1),
}
...
function DatabaseQueryClause.__static.Get(parent)
...
function DatabaseQueryClause.__private:_Acquire(parent)
	self._parent = parent
end
...
function DatabaseQueryClause:Function(field, func)
	return self:_SetComparisonOperation("FUNCTION", field, func)
end
...
function DatabaseQueryClause:_IsTrue(row, ignoreField)
	...
	elseif operation == "IS_NOT_NIL" then
		return row[self._field] ~= nil
	elseif operation == "FUNCTION" then
		return self._value(row[self._field]) and true or false
...
function DatabaseQueryClause:_SetComparisonOperation(operation, field, value, otherField)
	...
	assert(value == Util.CONSTANTS.OTHER_FIELD_QUERY_PARAM or not otherField)
