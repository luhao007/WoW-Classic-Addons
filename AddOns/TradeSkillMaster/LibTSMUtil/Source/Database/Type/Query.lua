...
local Row = LibTSMUtil:Include("Database.Type.Row")
...
function DatabaseQuery:VirtualField(field, fieldType, func, argField, defaultValue)
	...
	elseif argField and not self:_HasField(argField) then
...
---@param field string The name of the field
---@param func fun(value: any): boolean The function which gets passed the field value and returns whether or not the query results should include it
---@return DatabaseQuery
function DatabaseQuery:Function(field, func)
...
---@param func fun(row: DatabaseRow, arg: any): boolean The function which gets passed the row being evaulated and
...
function DatabaseQuery:GetSingleResultAndRelease()
	self:_Execute()
	assert(self:Count() == 1)
	return self:GetFirstResultAndRelease()
end

---Get the first result or selected fields.
---@return DatabaseRow|any
function DatabaseQuery:GetFirstResult()
	return self:GetNthResult(1)
end

---Get the first result or selected fields and releases the query.
---
---Note that this method internally iterates over all the results.
---@return DatabaseRow|any
function DatabaseQuery:GetFirstResultAndRelease()
	return self:GetNthResultAndRelease(1)
end

---Get the n-th result or selected fields.
---@return DatabaseRow|any
...
---Get the n-th result or selected fields and releases the query.
---@return DatabaseRow|any
function DatabaseQuery:GetNthResultAndRelease(n)
	if #self._select > 0 then
		return self:_PassThroughReleaseHelper(self:GetNthResult(n))
	else
		local row = self:GetNthResult(n)
		row = row and row:Clone() or nil
		self:Release()
		return row
	end
end
...
---@param field? string The field within the results (or nil to use a single selected field)
---@return number|nil
function DatabaseQuery:Min(field)
	self:_Execute()
	if not field then
		assert(#self._select == 1)
		field = self._select[1]
	end
	local result = nil
	for _, uuid in ipairs(self._result) do
...
---Gets the minimum value of a specific field within the query results (or nil if there are no results) and releases the query.
---@param field? string The field within the results (or nil to use a single selected field)
---@return number|nil
function DatabaseQuery:MinAndRelease(field)
	local result = self:Min(field)
	self:Release()
	return result
end
...
function DatabaseQuery:Hash(fields)
	...
	if fields then
		...
		for _, value in ipairs(hashContext) do
			result = Hash.Calculate(value, result)
		end
...
---Calculates the hash of the query results and release (or nil if there are no results).
---
---Note that either `fields` must be specified or the query must have a select colum with at most 2 fields.
...
---Deletes all the result rows from the database, releases the query, and returns the number of rows deleted.
---@return number
...
---@return boolean? @Whether or not the sort is ascending
...
function DatabaseQuery.__private:_ValidateEqualityValue(field, value, otherField)
	...
	elseif value ~= Util.CONSTANTS.BOUND_QUERY_PARAM then
		if enumFieldType then
			assert(EnumType.IsValue(value, enumFieldType))
		else
...
function DatabaseQuery.__private:_NewClause()
	assert(self._iteratorState == "IDLE")
...
function DatabaseQuery.__private:_Execute()
...
function DatabaseQuery.__private:_GetQueryIndexInfo()
...
function DatabaseQuery.__private:_PassThroughReleaseHelper(...)
	self:Release()
	return ...
end
	...
	if self._virtualFieldFunc[field] then
		...
		if argField then
			...
			if not self._tempVirtualResultRow then
				self._tempVirtualResultRow = Row.Get()
...
function DatabaseQuery.__private:_JoinHelper(joinType, db, field, foreignField, aggregateQuery)
	...
	elseif self._db:_HasField(field) then
		assert(not self._db:_GetListFieldType(field), "Can't join on list fields")
		local fieldType, enumFieldType = self._db:_GetFieldType(field)
	...
	if db:_HasField(foreignField) then
		assert(not db:_GetListFieldType(foreignField), "Can't join on list fields")
