...
function DatabaseTable.__private:__init(schema)
	...
	for _, fieldName, fieldType, isIndex, isUnique, isTrigram, smartMap, smartMapInput, fieldTypeEnumType in schema:_FieldIterator() do
		if smartMap then
...
---Get a unique row (or nil if no row exists).
...
---Get a single field from a unique row (or nil if no row exists).
---@param uniqueField string The unique field
---@param uniqueValue any The value of the unique field
...
---Get multiple fields from a unique row (or nil if no row exists).
---@param uniqueField string The unique field
---@param uniqueValue any The value of the unique field
...
function DatabaseTable:SetUniqueRowField(uniqueField, uniqueValue, field, value)
	self:_ValidateFieldValue(uniqueField, uniqueValue)
	self:_ValidateUnique(uniqueField)
	self:_ValidateFieldValue(field, value)
	if self:_GetListFieldType(field) then
		error("Cannot set list field using this method", 3)
...
---Delete a row by a unique field value.
---@param uniqueField string The unique field
---@param uniqueValue any The value of the unique field
function DatabaseTable:DeleteRowByUniqueField(uniqueField, uniqueValue)
	self:_ValidateFieldValue(uniqueField, uniqueValue)
	self:_ValidateUnique(uniqueField)
	local uuid = self:_GetUniqueRow(uniqueField, uniqueValue)
	assert(uuid)
	self:DeleteRowByUUID(uuid)
end
...
function DatabaseTable:BulkInsertNewRow(...)
	...
	for i = 1, numFields do
		...
		elseif fieldTypeEnumType then
			if not EnumType.IsValue(value, fieldTypeEnumType) then
...
function DatabaseTable.__private:_UpdateQueries(uuid, oldValues)
...
function DatabaseTable.__private:_TrigramIndexRemove(uuid)
