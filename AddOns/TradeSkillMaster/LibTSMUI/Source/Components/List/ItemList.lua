...
local Hash = LibTSMUI:From("LibTSMUtil"):Include("Util.Hash")
...
local ItemList = UIElements.Define("ItemList", "List")
...
function ItemList:_HandleQueryUpdate(query)
	...
	if not isCollapsed and #oldUUIDs > 0 and query:UUIDDiffPrepare(oldUUIDs) then
		...
		local prevSelectionHash = Hash.Calculate(self._selectedData)
		for itemString in pairs(self._selectedData) do
		...
		selectionChanged = Hash.Calculate(self._selectedData) ~= prevSelectionHash
	end
...
function ItemList.__protected:_HandleRowAcquired(row)
	...
	expander:SetPoint("LEFT", colSpacing / 2, 0)
	check:SetPoint("LEFT", (expander:GetWidth() + INDENT_WIDTH - check:GetWidth() + colSpacing / 2), 0)
...
function ItemList.__private:_DrawHeadingCollapsedState(row, isCollapsed)
	local expander = row:GetTexture("expander")
	expander:TSMSetTexture(isCollapsed and EXPANDER_TEXTURE_COLLAPSED or EXPANDER_TEXTURE_EXPANDED)
end
...
function ItemList.__protected:_HandleRowMouseDown(row, mouseButton)
