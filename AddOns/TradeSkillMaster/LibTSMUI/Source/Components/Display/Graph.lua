...
local Graph = UIElements.Define("Graph", "Element")
Graph:_ExtendStateSchema()
	:AddBooleanField("mouseOver", false)
	:AddOptionalNumberField("selectionStartX")
...
function Graph:__init()
	...
	frame.plot = self:_CreateFrame(frame)
	...
	frame.plot.dot = self:_CreateTexture(frame.plot, "ARTWORK", 3)
	frame.plot.dot:TSMSetTextureAndSize("uiFrames.HighlightDot")

	frame.plot.hoverLine = self:_CreateTexture(frame.plot, "ARTWORK", 2)
	frame.plot.hoverLine:SetWidth(HOVER_LINE_THICKNESS)
	frame.plot.hoverLine:Hide()

	frame.plot.hoverText = self:_CreateFontString(frame.plot)
	frame.plot.hoverText:SetSize(PLOT_HIGHLIGHT_TEXT_WIDTH, PLOT_HIGHLIGHT_TEXT_HEIGHT)
	frame.plot.hoverText:Hide()

	frame.plot.selectionBox = self:_CreateTexture(frame.plot, "ARTWORK", 2)
...
function Graph:Acquire()
	...
	frame.plot.hoverLine:TSMSubscribeColorTexture("INDICATOR_ALT")
	...
	self._state:PublisherForKeys("mouseOver", "selectionStartX")
...
function Graph.__private:_ComputeData(plotWidth, plotHeight)
	wipe(self._xValuesFiltered)
	wipe(self._yLookup)
	wipe(self._xAxisValues)
	wipe(self._yAxisValues)
...
function Graph.__private:_CreateAxisLine(axis)
	local plot = self:_GetBaseFrame().plot
	local line = self:_CreateTexture(plot, "BACKGROUND")
	line:SetTexture("Interface\\AddOns\\TradeSkillMaster\\Media\\line.tga")
	line:TSMSubscribeVertexColor("ACTIVE_BG")
	line.text = self:_CreateFontString(plot)
	line.text:TSMSubscribeTextColor("TEXT")
	if axis == "X" then
...
function Graph.__private:_DrawYAxisLine(line, offset, value, plotWidth, plotHeight, suggestedStep)
	local plot = self:_GetBaseFrame().plot
...
function Graph.__private:_DrawXAxisLine(line, offset, value, plotWidth, plotHeight, suggestedStep)
	local plot = self:_GetBaseFrame().plot
...
function Graph.__private:_CreateSlice()
	local plot = self:_GetBaseFrame().plot
	local slice = self:_CreateTexture(plot, "ARTWORK")
	slice:SetTexture("Interface\\AddOns\\TradeSkillMaster\\Media\\line.tga")
	slice.fillTop = self:_CreateTexture(plot, "ARTWORK", -1)
	slice.fillTop:SetTexture("Interface\\AddOns\\TradeSkillMaster\\Media\\triangle")
	slice.fillBar = self:_CreateTexture(plot, "ARTWORK", -1)
	slice.fillBar:SetTexture("Interface\\Buttons\\WHITE8X8")
	return slice
end

function Graph.__private:_LayoutSlice(slice, xFrom, yFrom, xTo, yTo, thickness)
	assert(xFrom <= xTo)
	local plot = self:_GetBaseFrame().plot
	local textureHeight = thickness * LINE_THICKNESS_RATIO
...
function Graph.__private:_SetSliceColor(slice, color, y1, y2, plotHeight)
	local colorObj = Theme.GetColor(color)
	local r, g, b, a = colorObj:GetFractionalRGBA()
	...
	slice.fillTop:TSMSetVerticalGradient(colorObj, barMaxAlpha, topMaxAlpha)
	slice.fillBar:TSMSetVerticalGradient(colorObj, 0, barMaxAlpha)
end

function Graph.__private:_GetCursorClosestPoint()
	local plot = self:_GetBaseFrame().plot
...
function Graph.__private:_XValueToPlotCoord(xValue)
	local plot = self:_GetBaseFrame().plot
