local ADDON_NAME, namespace = ... 	--localization
local L = namespace.L 				--localization

--local LOCALE = GetLocale()

if namespace.locale == "frFR" then
	-- The EU English game client also
	-- uses the US English locale code.

-- ####################################################################################################################################
-- ##	Français (French) translations provided by Lightuky, Medaleux, sv002, Darkcraft92, Ymvej, Druidzor, and napnapnapnapnap on Curseforge. ##
-- ####################################################################################################################################

L["  /dcstats config: Opens the DejaClassicStats addon config menu."] = "/dcstats config: Ouvre le menu de configuration de DejaClassicStats."
L["  /dcstats reset:  Resets DejaClassicStats options to default."] = "/dcstats reset:  Réinitialise les options de DejaClassicStats par défaut."
L["%s of %s increases %s by %.2f%%"] = "%s de %s augmente %s par %.2f%%"
L["About DCS"] = "À propos de DCS"
L["All Stats"] = "Toutes les stats"
L["Attack"] = "Attaque"
L["Average Durability"] = "Durabilité Moyenne"
L["Average equipped item durability percentage."] = "Pourcentage de durabilité moyen des objets équipés."
L["Average Item Level:"] = "Moyenne du niveaux d'équipement:"
L["Avoidance Rating"] = "Taux d'évitement"
L["Blizzard's Hide At Zero"] = "Blizzard Cacher valeur nulle "
L["Character Stats:"] = "Statistiques de personnage:"
L["Class Colors"] = "Couleur par classe "
L["Class Crest Background"] = "Arrière plan de classe"
L["Critical Strike Rating"] = "Score de coup critique"
L["DCS's Hide At Zero"] = "DCS Cacher valeur nulle "
L["Decimals"] = "Décimales"
L["Defense"] = "Défense"
L["Dejablue's improved character stats panel view."] = "Vue améliorée des statistiques du personnage de Dejablue"
L["DejaClassicStats Slash commands (/dcstats):"] = "Commandes de DCS (/dcstats)"
L["Displays a durability bar next to each item."] = "Afficher une barre de durabilité à côté de chaque objet."
L["Displays average item durability on the character shirt slot and durability frames."] = "Afficher la durabilité moyenne des objets équipés sur l'emplacement de chemise et sur l'affichage de la durabilité."
L["Displays average item level to one decimal place."] = "Afficher le niveau d'objet moyen avec une décimale."
L["Displays average item level to two decimal places."] = "Affiche le niveau d'objet avec deux décimales"
L["Displays average item level with class colors."] = "Montrer le niveaux d'équipement de la couleur de la classe"
L["Displays each equipped item's durability."] = "Afficher la durabilité de chaque objet équipé."
L["Displays each equipped item's repair cost."] = "Afficher le coût en réparations de chaque objet équipé."
L["Displays 'Enhancements' category stats to two decimal places."] = "Afficher les statistiques de la catégorie 'Améliorations' avec deux décimales."
L["Displays Equipped/Available item levels unless equal."] = "Afficher niveau d'objet Equipé/Disponible, sauf s'ils sont égaux."
L["Displays the class crest background."] = "Afficher le blason de classe en arrière plan"
L["Displays the DCS scrollbar."] = "Afficher la barre de défilement de DCS."
L["Displays the Expand button for the character stats frame."] = "Afficher le bouton Étendre pour la fenêtre de stats du personnage."
L["Displays the item level of each equipped item."] = "Affiche le niveau de chaque objet équipé"
L["Dodge Rating"] = "Score d'esquive"
L["Durability"] = "Durabilité"
L["Durability Bars"] = "Barres de durabilité"
L["Equipped/Available"] = "Équipé/Disponible"
L["Expand"] = "Étendre"
L["General"] = "Général "
L["General global cooldown refresh time."] = "Temps de rafraîchissement du temps de recharge global "
L["Global Cooldown"] = "Temps de recharge global"
L["Haste Rating"] = "Score d'hâte"
L["Hide Character Stats"] = "Cacher les statistiques du personnage"
L["Hide low level mastery"] = "Cacher la maîtrise à bas niveau "
L["Hides 'Enhancements' stats if their displayed value would be zero. Checking 'Decimals' changes the displayed value."] = [=[Cacher les caractéristiques 'd'améliorations' si leur valeurs affichées sont égales à zéro. 
Cocher 'Décimales' change la valeur affichée.]=]
L["Hides 'Enhancements' stats only if their numerical value is exactly zero. For example, if stat value is 0.001%, then it would be displayed as 0%."] = "Cacher les caractéristiques 'd'améliorations' seulement si leurs valeurs numérique sont exactement égales à zéro. Si la valeur est égale à 0.001%, elle sera affichée comme 0%."
L["Hides Mastery stat until the character starts to have benefit from it. Hiding Mastery with Select-A-Stat™ in the character panel has priority over this setting."] = "Cacher la caractéristique de Maîtrise jusqu'à ce que le personnage en bénéficie. Cacher la Maîtrise avec Select-A-Stat™ sur le panneau de personnage est prioritaire sur ce règlage. "
L["Item Durability"] = "Durabilité de l'objet"
L["Item Level"] = "Niveau d'équipement"
L["Item Repair Cost"] = "Coût de réparation de l'objet"
L["Item Slots:"] = "Emplacement d'équipement:"
L["Leech Rating"] = "Score de ponction"
L["Lock DCS"] = "Vérouiller DCS"
L["Main Hand"] = "Main principale"
L["Mastery Rating"] = "Score de maîtrise"
L["Miscellaneous:"] = "Divers:"
L["Movement Speed"] = "Vitesse de déplacement"
L["Off Hand"] = "Main gauche"
L["Offense"] = "Attaque"
L["One Decimal Place"] = "Afficher une décimale"
L["Parry Rating"] = "Score de parade"
L["Ratings"] = "Taux"
L["Relevant Stats"] = "Stats pertinentes"
L["Repair Total"] = "Réparation Total"
L["Requires Level "] = "Requiert le niveau"
L["Reset Stats"] = "Réinitialiser"
L["Reset to Default"] = "Remettre par défaut"
L["Resets order of stats."] = "Réinitialise l'ordre des caractéristiques"
L["Scrollbar"] = "Barre de défilement"
L["Show all stats."] = "Montrer toutes les statistiques"
L["Show Character Stats"] = "Montrer les statistiques du personnage"
L["Show only stats relevant to your class spec."] = "Afficher seulement les caractéristiques adaptées à votre spécialisation de classe"
L["Total equipped item repair cost before discounts."] = "Coût total de réparation des objets équipés avant réductions."
L["Two Decimal Places"] = "Afficher deux décimales"
L["Unlock DCS"] = "Déverouiller DCS"
L["Versatility Rating"] = "Score de polyvalence"
L["weapon auto attack (white) DPS."] = "DPS de l'Auto-attaque de l'arme (blanc)"
L["Weapon DPS"] = "DPS de l'arme"

----------------------------------------------------
-- DejaClassicStats specific translation phrases. --
----------------------------------------------------
L[". Critical Hit immunity for a level 60 player against a raid boss occurs at 440 Defense and requires a defense skill of 140 from items and enhancements to achieve."] = ". L'immunité aux Coups Critiques pour un joueur de niveau 60 contre un boss de raid se produit à 440 Défense dont 140 de Défense qui viennent des objets et améliorations."
L["+ Arcane: "] = "+ Arcane:"
L["+ Fire: "] = "+ Feu:"
L["+ Healing: "] = "+ Soins: "
L["+ Holy: "] = "+ Sacré:"
L["+ Nature: "] = "+ Nature:"
L["Alternate Expand"] = "\"Étendre\" Alternatif"
L["Attack Speed (seconds): "] = "Vitesse d'attaque (secondes):"
L["Background Art"] = "Art de fond"
L["Base Defense including talents such as Warrior's Anticipation is "] = "La Défense de base avec les talents tels qu'Anticipation des Guerriers est"
L["Black Item Icons"] = "Noircir les icônes d'objets"
L["Black item icons to make text more visible."] = "Noircir les icônes d'objets pour rendre le texte plus visible."
L["Block: "] = "Blocage: "
L["Bonus Defense from items and enhancements is "] = "La Défense supplémentaire des objets et améliorations est "
L["Categories:"] = "Catégories: "
L["Damage per Second: "] = "Dégâts par seconde:"
L["Damage: "] = "Dégâts:"
L["Darken Item Icons"] = "Assombrir les icônes d'objets"
L["Darken item icons to make text more visible."] = "Assombrir les icônes d'objets pour rendre le texte plus visible."
L["Default Resistances"] = "Résistance par défaut"
L["Default Stats"] = "Stats par défaut"
L["Defense: "] = "Défense: "
L["Display Info Beside Items"] = "Afficher les infos à côté des objets"
L["Displays black and white class talents background art."] = "Définir le fond des talents de classe en noir et blanc."
L["Displays each equipped item's enchantment."] = "Affiche les enchantements de chaque objets."
L["Displays the class talents background art."] = "Affiche l'art de fond des talents de classe."
L["Displays the Expand button above the hands item slot."] = "Affiche le bouton \"Étendre\" au-dessus de l'emplacement de l'objet de main."
L["Displays the item's info beside each item's slot."] = "Affiche les informations sur l'objet à côté de l'emplacement de chaque objets."
L["Dodge: "] = "Esquive: "
L["Durability: "] = "Durabilité: "
L["Enchants"] = "Enchantements"
L["Main Hand Damage: "] = "Dégâts Main Droite"
L["Main Hand: "] = "Main principale: "
L["Mana Regen Current: "] = "Régén. Mana actuelle: "
L["Mana Regen: "] = "Régén. Mana: "
L["Melee +Damage: "] = "+Dégâts en mêlée:  "
L["Melee Crit: "] = "Critique (mêlée): "
L["Melee Enhancements"] = "Stats Corps-à-corps"
L["Melee Hit: "] = "\"Toucher (mêlée): "
L["Monochrome Background Art"] = "Art de fond unicolore"
L["Movement Speed: "] = "Vitesse de mouvement: "
L["MP5: "] = "MP5: "
L["Off Hand Damage: "] = "Dégâts Main Gauche:"
L["Off Hand: "] = "Main gauche: "
L["Parry: "] = "Parade: "
L["Physical Critical Strike: "] = "Coup Critique physique: "
L["Power: "] = "Puissance:"
L["Primary"] = "Stats Principales"
L["Ranged Crit: "] = "Critique (distance): "
L["Ranged Damage: "] = "Dégâts à Distance:"
L["Repair Total: "] = "Réparations: "
L["Rotation Buttons"] = "Boutons de rotation"
L["Spell +Damage: "] = "+Dégâts sorts: "
L["Spell Crit: "] = "Critique (sorts): "
L["Spell Enhancements"] = "Stats des Sorts"
L["Spell Hit: "] = "Toucher (sorts): "
L["Total Defense is "] = "La Défense totale est "

return end
