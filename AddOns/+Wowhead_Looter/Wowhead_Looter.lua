...
local WL_ADDON_UPDATED = "2025-09-23";

local WL_NAME = "|cffffff7fWowhead Looter|r";
local WL_VERSION = 50501;
...
local WL_SPECIAL_CONTAINERS = {
    ...
    -- Mists Classic
    [248666] = true, -- Satchel of Celestial Chance
};
...
function wlCheckUnitForRep(guid, name)
    ...
    local uiMapID = wlGetCurrentUiMapID();
    local taskInfo = C_TaskQuest and C_TaskQuest.GetQuestsOnMap and C_TaskQuest.GetQuestsOnMap(uiMapID);
    local numTaskPOIs = 0;
...
local wlSpells = {
    ...
    CelestialSatchel = { GetSpellInfo(1251365) or "", WL_ITEM, 1 },
...
function wlFindSpell(name)
    for k,v in pairs(wlSpells) do
        if name == v[1] or name:match("^"..v[1]) then
            return k;
        end
    end
...
function wlEvent_CURRENCY_DISPLAY_UPDATE(...)
    ...
    elseif currencyId and currencyAmount and wlTracker.spell and wlTracker.spell.id and
        wlIsValidInterval(wlTracker.spell.time or 0, now, 500) and
        WL_SPECIAL_CONTAINERS[wlTracker.spell.id] then
        ...
        if not eventId then
            ...
            wlEvent[wlId][wlN][eventId].what = "loot";
            wlTableCopy(wlEvent[wlId][wlN][eventId], wlTracker.spell);
            wlEvent[wlId][wlN][eventId].id = wlTracker.spell.id;
            wlEvent[wlId][wlN][eventId].dd = wlGetInstanceDifficulty();
            wlEvent[wlId][wlN][eventId].flags = 0;
        end
        wlUpdateVariable(wlEvent, wlId, wlN, eventId, "drop", 1, "set", wlConcat("currency", currencyAmount, currencyId));
...
function wlGetExportDataValue()
    local value = "&realmId="..wlGetPlayerRealmId().."&timePlayedTotal="..wlScans.timePlayedTotal.."&achievements="..wlScans.achievements.."&toys="..wlScans.toys.."&mounts="..wlScans.mounts.."&titles="..wlScans.titles.."&heirlooms="..wlScans.heirlooms.."&appearances="..wlScans.appearances;
...
function wlEvent_CVAR_UPDATE(arr, name, val)
    if name == 'minimapZoom' then
        wlUpdateMiniMapButtonPosition(_G["wlMinimapButton"]);
    end
end
...
local wlEvents = {
    ...
    CVAR_UPDATE = wlEvent_CVAR_UPDATE,
