...
function public.scan()
    ...
    savedData[key].talents = private.getMistsTalent();
    savedData[key].spec = private.getMistsSpec();
    savedData[key].glyphs = private.getMistsGlyphs();
end
...
--- Returns a list of active glyph spell IDs.
function private.getMistsGlyphs()
    local result = {};
    local groupIndex = C_SpecializationInfo.GetActiveSpecGroup();

    for i = 1, NUM_GLYPH_SLOTS do
        local enabled, glyphType, _, glyphSpell = GetGlyphSocketInfo(i, groupIndex);
        if glyphSpell then
            table.insert(result, glyphSpell);
        end
    end
...
function private.getMistsSpec()
    local specIndex = C_SpecializationInfo.GetSpecialization();

    -- 5 are the "Initial" specs, meaning no spec is selected.
    if specIndex ~= 5 then
...
--- Returns the Wowhead-style talent string for the current talent build.
function private.getMistsTalent()
    local result = {};
    local groupIndex = C_SpecializationInfo.GetActiveSpecGroup();
...
function private.init()
    private.eventFrame:RegisterEvent("GLYPH_ADDED");
    private.eventFrame:RegisterEvent("GLYPH_REMOVED");
    ...
    private.eventFrame:RegisterEvent("PLAYER_SPECIALIZATION_CHANGED");
    private.eventFrame:RegisterEvent("PLAYER_TALENT_UPDATE");
    private.eventFrame:RegisterEvent("CHARACTER_POINTS_CHANGED");
