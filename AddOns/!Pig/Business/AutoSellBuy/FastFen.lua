...
function BusinessInfo.FastFen()
	...
	local FenItemKey = CreateFrame("Button","FenItemKey",UIParent, "SecureActionButtonTemplate");
	PIGUseKeyDown(FenItemKey)
	FenItemKey:SetAttribute("type", "macro")
	local function zhixingClick(self,button)
		if not IsPlayerSpell(13262) then PIGinfotip:TryDisplayMessage("你尚未学会"..gongnengName.."技能") return end
	...
	FenItemKey:HookScript("PreClick",  function (self,button)
	...
	fujiF.Fen_Tishi = PIGCheckbutton(fujiF,{"TOPLEFT",fujiF,"TOPLEFT",10,-6},{"提示"..gongnengName, "有可"..gongnengName.."物品将会在"..L["ACTION_TABNAME2"].."按钮提示"})
	fujiF.Fen_Tishi:SetScript("OnClick", function (self)
		if self:GetChecked() then
			PIGA["AutoSellBuy"]["Fen_Tishi"]=true;
		else
			PIGA["AutoSellBuy"]["Fen_Tishi"]=false;
		end
	end);
	fujiF.Fen_Tishi:SetChecked(PIGA["AutoSellBuy"]["Fen_Tishi"])
	...
	fujiF.QkBut:SetChecked(PIGA["AutoSellBuy"]["Fen_QkBut"])
	...
	local function ADD_kaiqiHong()
		...
		if UseKeyDown=="0" then
			hongNR = [=[/click FenItemKey LeftButton 0]=]
		elseif UseKeyDown=="1" then
			hongNR = [=[/click FenItemKey LeftButton 1]=]
		end
		local macroSlot = GetMacroIndexByName(hongName)
		if macroSlot>0 then
