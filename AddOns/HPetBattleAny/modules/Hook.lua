...
HPetBattleAny.hook_PJ = hookPetJournal ------快速自定义宠物链接
--
...
if HPetBattleAny.CreateLinkByInfo then
    ...
    HP_L = function(id, mlevel, mhealth, mpower, mspeed, mrarity)
        id, mlevel, mhealth, mpower, mspeed, mrarity = tonumber(id), tonumber(mlevel), tonumber(mhealth),
            tonumber(mpower), tonumber(mspeed), tonumber(mrarity)
        ...
        elseif C_PetJournal.GetPetInfoBySpeciesID(id) == id then
            ...
            print(HPetBattleAny.ICON_LIST[h] ..
                "\124cffffff00\124Hbattlepet:" ..
                id ..
                ":0:0:0:0:0:BattlePet-0-000000000000:0\124h[" ..
                (C_PetJournal.GetPetInfoBySpeciesID(id) or "") .. "]\124h\124r")
        end
    end
end

-------收集信息(目标提示)
local function TooltipAddOtherInfo(tooltip, speciesID)
    if HPetSaves.Tooltip then
        local sourceText = select(5, C_PetJournal.GetPetInfoBySpeciesID(speciesID))
        if sourceText and sourceText ~= "" then
            --清除sourceText最后可能存在的"\n"
            if sourceText:sub(#sourceText, #sourceText) == "n" then
                sourceText = sourceText:sub(1, #sourceText - 2)
            end
            local pets = HPetBattleAny:GetPetInfo(speciesID)
            if pets then
                local str1, str2 = HPetBattleAny:GetPetCollectedInfo(pets)
                tooltip:AddDoubleLine(string.trim(str2), nil, 1, 1, 1)
            end
...
local function GetOwnedInfo(owned, speciesID, b)
    ...
    if sourceText and sourceText ~= "" then
        ...
        for i = 1, HPetBattleAny:GetPetNum(speciesID) do
            ...
            local str = ""
            if rarity then
                str = str .. ITEM_QUALITY_COLORS[rarity - 1].hex .. level .. "|r"
            end
            if breedID then
                str = str .. HPetBattleAny.ICON_LIST[breedID]
            end
            if b then
...
HPetBattleAny.GetOwnedInfo = GetOwnedInfo
...
--	initPetJournal需要跟随加载/initOther启动加载
...
hookPetJournal.init = function()
    ...
    if PetJournal then
        for i, button in pairs(PetJournal.ScrollBox:GetFrames()) do
            _hook(button)
        end
    else
        hooksecurefunc(
    ...
    PetJournal.Loadout.Pet1.dragButton:HookScript("OnEnter", hookPetJournal.PJSTooltip)
    PetJournal.Loadout.Pet2.dragButton:HookScript("OnEnter", hookPetJournal.PJSTooltip)
    PetJournal.Loadout.Pet3.dragButton:HookScript("OnEnter", hookPetJournal.PJSTooltip)
    ...
    ---------------手动xxxxxxxxxx
    for i = 1, 6 do
        _G["PetJournalPetCardSpell" .. i]:HookScript(
            "OnClick",
            function(self)
                ...
                if (not IsModifiedClick() and not self.LevelRequirement:IsShown()) or (IsModifiedClick() and self.LevelRequirement:IsShown()) then
                    local pos = string.gsub("%1%2%3", "(.*)(%%" .. ((i > 3) and (i - 3) or i) .. ")(.*)",
                        "%1" .. i .. "%3")
                    if pos then
                        HPetSaves.PetAbilitys[PetJournalPetCard.petID] = string.gsub(
                            HPetSaves.PetAbilitys[PetJournalPetCard.petID] or "000", "^(.)(.)(.)$", pos)
                    end
...
hookfunction.init = function()
    ...
    TooltipDataProcessor.AddTooltipPostCall(
        ...
        function(tooltip, tooltipData)
            if not HPetSaves.Tooltip or not tooltip.GetUnit then
                return
            end
            ...
            if unit and UnitIsBattlePet and UnitIsBattlePet(unit) then
                ...
                TooltipAddOtherInfo(tooltip, speciesID)
            end
        end
    )
    ...
    -- )
    -----(物品/Item)
    local function BattlePetItemTooltip(tooltip, tooltipData)
        if tooltipData.id then
            ...
            if petID and type(petID) == "number" then
                ...
                tooltip:AddLine("|cffff0000" .. NOT_COLLECTED .. "!|r")
                tooltip:Show()
            end
        end
    end
    TooltipDataProcessor.AddTooltipPostCall(Enum.TooltipDataType.Item, BattlePetItemTooltip)
    ...
    C_PetJournal.GetOwnedBattlePetString = function(id)
        ...
        if not str then
            return GREEN_FONT_COLOR_CODE ..
                format(ITEM_PET_KNOWN, C_PetJournal.GetNumCollectedInfo(id)) .. FONT_COLOR_CODE_CLOSE
        else
            return str
        end
    end
    ...
    -----------------
end --抓获宠物得到的链接，petid缺少"0x"，ItemRef:255,FloatingBattlePet_Toggle --PetJournal_SelectPet(PetJournal,"0x000000000124D362") ------	点击链接无法正确指向宠物(修复处2) ------  2022-11-16 屏蔽,不需要了 -- hookPetJournal.PetJournal_SelectPet = function(self, targetPetID,...) -- 	local numPets = C_PetJournal.GetNumPets(PetJournal.isWild); -- 	local petIndex = nil; -- 	local targetPetID = targetPetID -- 	for i = 1,numPets do -- 		local petID, speciesID, owned = C_PetJournal.GetPetInfoByIndex(i, PetJournal.isWild); -- 		if (petID == targetPetID) then -- 			petIndex = i; -- 			break;
--petjournal
...
hookPetJournal.PetJournal_UpdatePetCard = function(self)
    ...
    if PetJournalPetCard.speciesID then
        ...
        if HPetSaves.ShowHideID then
            PetJournalPetCard.TypeInfo.type:SetText("(" ..
                PetJournalPetCard.speciesID .. ")" .. _G["BATTLE_PET_NAME_" .. petType] .. strbreedID)
        else
...
hookPetJournal.SetAbility = function(loadoutID, index, abilityID, stop)
    if not HPetSaves.AutoSaveAbility or stop then
        return
    end
    if not HPetSaves.PetAbilitys then
...
hookfunction.PetBattleUnitTooltip_UpdateForUnit = function(self, petOwner, petIndex)
    local r, g, b, hex = C_Item.GetItemQualityColor(C_PetBattles.GetBreedQuality(petOwner, petIndex))
    local speciesID = C_PetBattles.GetPetSpeciesID(petOwner, petIndex)
    local name, icon, petType, creatureID, sourceText, description, isWild, canBattle, tradable = C_PetJournal
        .GetPetInfoBySpeciesID(speciesID)
    local strbreedID = ""
    ------- 成长值
    ...
    -----	加入收集信息
    TooltipAddOtherInfo(GameTooltip, speciesID)

    ----- 	技能CD
    if true then
        self.AbilitiesLabel:Show()
        local enemyPetType = C_PetBattles.GetPetType(PetBattleUtil_GetOtherPlayer(petOwner),
            C_PetBattles.GetActivePet(PetBattleUtil_GetOtherPlayer(petOwner)))
        for i = 1, NUM_BATTLE_PET_ABILITIES do
            local id, name, icon, maxCooldown, description, numTurns, abilityPetType, noStrongWeakHints = C_PetBattles
                .GetAbilityInfo(petOwner, petIndex, i)
            if not id and (not C_PetBattles.IsPlayerNPC() and petOwner == 1) and HPetBattleAny.AbilityModules.GetAbilityInfo then
                id, name, icon, maxCooldown, description, numTurns, abilityPetType, noStrongWeakHints = HPetBattleAny
                    .AbilityModules.GetAbilityInfo(petOwner, petIndex, i, true)
            end
    ...
    if (nextFrame > 1) then
        ...
        if not self.Debuffs:IsShown() then
...
hookfunction.PetBattleUnitFrame_UpdateDisplay = function(self)
    if self.petOwner and self.petIndex and self.petIndex <= C_PetBattles.GetNumPets(self.petOwner) then
        local r, g, b = C_Item.GetItemQualityColor(C_PetBattles.GetBreedQuality(self.petOwner, self.petIndex))
        if self.Name then
...
hookfunction.FloatingBattlePet_Show = function(speciesID, level)
    ...
    if owned ~= nil then
        ...
        if str then
            FloatingBattlePetTooltip.Owned:SetText(str)
        end
    end
end
----- 战宠鼠标提示
hookfunction.BattlePetToolTip_Show = function(speciesID, level, breedQuality, maxHealth, power, speed, customName)
    ...
    if owned ~= nil then
        local str = GetOwnedInfo(owned, speciesID)
        if str then
            BattlePetTooltip.Owned:SetText(str)
        end
        ----- 修复该鼠标大小问题
...
hookfunction.BattlePetTooltipTemplate_SetBattlePet = function(frame, data)
    if data.level ~= 0 and data.breedQuality ~= -1 then
        local ghealth, gpower, gspeed, breedID = HPetBattleAny.ShowMaxValue(data.speciesID, data.level, data.maxHealth,
            data.power, data.speed, data.breedQuality + 1)
        ...
        if breedID and HPetSaves.ShowBreedID then
            if HPetSaves.BreedIDStyle then
                ...
                frame.PetType:SetText(format("%s(%s)", _G["BATTLE_PET_NAME_" .. data.petType],
                    HPetBattleAny.GetBreedNames[breedID]))
            end
        end
        if not frame.Name:GetText() then
            frame.Name:SetText(data.customName)
        end
    end
end
...
hookfunction.PetBattleUnitFrame_OnClick = function(self, button)
    if button == "LeftButton" then
        ...
        if speciesID then
            hookfunction.FloatingBattlePet_Show(speciesID, 0)
        end

        if self.petOwner == 1 then
            for i = 1, C_PetBattles.GetNumPets(self.petOwner) do
                local petID = C_PetJournal.GetPetLoadOutInfo(i)
                local tspeciesID, customName, level, xp, maxXp, displayID, isFavorite, name, icon, petType = C_PetJournal
                    .GetPetInfoByPetID(petID)
        ...
        elseif self.petOwner == 2 then
            if HPetAllInfoFrame then
                ...
                if (HPetBattleAny.EnemyPetInfo[self.petIndex]) then
                    breedID, rarity, level = HPetBattleAny.EnemyPetInfo[self.petIndex].breedID,
                        HPetBattleAny.EnemyPetInfo[self.petIndex][5], HPetBattleAny.EnemyPetInfo[self.petIndex][1]
                end
...
hookfunction.SharedPetBattleAbilityTooltip_SetAbility = function(self, abilityInfo, additionalTex)
    if not HPetSaves.ShowHideID then
        return
    end
...
HookMiniMap.changetxtfunc = function(value)
    ...
    if speciesID then
        ...
        if str then
            local icon = "\124T" ..
                "Interface\\Icons\\Pet_TYPE_" ..
                PET_TYPE_SUFFIX[select(3, C_PetJournal.GetPetInfoBySpeciesID(speciesID))] ..
                ":12:12:0:0:0:0:0:0:0:0\124t"
...
TooltipDataProcessor.AddTooltipPreCall(
    Enum.TooltipDataType.MinimapMouseover,
    function(tooltip, tooltipData)
        if tooltipData.lines and tooltipData.lines[1] then
            if tooltipData.lines[1].args then
                --deprecated
                for key, value in pairs(tooltipData.lines[1].args) do
                    if value.field == "leftText" then
                        local tooltiptxt = value.stringVal
                        if tooltiptxt and tooltiptxt ~= HookMiniMap.oldtext then
                            HookMiniMap.oldtext = string.gsub(tooltiptxt, "[^\n]+", HookMiniMap.changetxtfunc)
                            value.stringVal = HookMiniMap.oldtext
                        end
                    end
                end
            else
                --10.1
                local tooltiptxt = tooltipData.lines[1].leftText
                if tooltiptxt and tooltiptxt ~= HookMiniMap.oldtext then
                    HookMiniMap.oldtext = string.gsub(tooltiptxt, "[^\n]+", HookMiniMap.changetxtfunc)
                    tooltipData.lines[1].leftText = HookMiniMap.oldtext
                end
            end
        end
    end
