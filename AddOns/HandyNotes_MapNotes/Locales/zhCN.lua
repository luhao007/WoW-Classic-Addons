...
-- Worldmap
L["Worldmap player arrow"] = "世界地图玩家箭头"
...
L["< Shift + Left Click sets a waypoint on a MapNotes icon >"] = "< Shift + 左键点击可在 MapNotes 图标上设置航点 >"
L["< Shift + Right Click sets a waypoint on a MapNotes icon >"] = "< Shift + 右键点击可在 MapNotes 图标上设置航点 >"
L["When enabled, the function of the left and right mouse buttons is swapped"] = "启用后，鼠标左右键的功能将互换"
L["Sets a waypoint on a MapNotes icon"] = "在 MapNotes 图标上设置一个航点"
L["Opens the adventure guide of the corresponding instance"] = "打开对应副本的冒险指南"
L["Opens the map where the destination is"] = "打开目的地所在的地图"
L["< Right Click sets a waypoint on a MapNotes icon >"] = "< 右键点击在 MapNotes 图标上设置航点 >"
L["< Right Click to open Adventure Guide >"] = "< 右键点击打开冒险指南 >"
L["< Right Click to show map >"] = "< 右键点击显示地图 >"
L["< Left Click sets a waypoint on a MapNotes icon >"] = "< 左键点击在 MapNotes 图标上设置航点 >"
L["Right-clicking on a symbol on this map type opens the corresponding map in which the symbol is located"] = "右键点击此地图类型上的图标会打开相应的地图"
L["Right-clicking on a MapNotes raid (green), dungeon (blue) or multiple icon (green&blue) on the map opens the corresponding dungeon or raid map in the Adventure Guide"] = "在地图上右键点击 MapNotes 的团队副本（绿色）、地下城（蓝色）或多个图标（绿+蓝）将打开冒险指南中的相应地图"
L["Right-clicking on a multiple icon will open the map where the dungeons are located"] = "右键点击多重图标会打开地下城所在的地图"
-- Races
...
L["Displays the player arrow on the minimap layered above addon-created icons"] = "在小地图上显示玩家箭头，位于插件创建的图标之上"
L["Coordinates"] = "坐标"
L["Two mouse coordinate windows are created on the world map: one on the minimized world map and one on the maximized world map"] = "世界地图上会创建两个鼠标坐标窗口：一个在缩小的世界地图上，另一个在放大的世界地图上"
L["The mouse coordinate windows share size and transparency, but can be positioned differently"] = "鼠标坐标窗口共享大小和透明度，但可以放置在不同位置"
L["Creates a window for displaying the coordinates"] = "创建用于显示坐标的窗口"
...
L["Shift + Right Click on a MapNotes icon adds a waypoint to it"] = "按住Shift键并右键点击MapNotes图标添加一个路标"
L["Too many boss names – click on this icon and then choose one of the dungeons or raids"] = "首领名称过多 – 点击此图标后选择一个地下城或团队副本"
L["If TomTom is installed, a TomTom waypoint is created. If TomTom is not installed, a waypoint is created using the Blizzard waypoint system"] = "如果安装了TomTom，将创建一个TomTom路径点。如果没有安装TomTom，将使用暴雪的路径点系统创建路径点。"
-- create and copy links
L["Use CTRL + C to copy the link"] = "使用 CTRL + C 复制链接"
L["Enable „Link“ in MapNotes („General“ - „Chat-Options“)"] = "在 MapNotes 中启用“链接”（“常规” - “聊天选项”）"
L["to create clickable quest or achievement links in chat"] = "在聊天中创建可点击的任务或成就链接"
L["Activate the 'Link' function in the MapNotes menu to generate a clickable web link"] = "在 MapNotes 菜单中启用“链接”功能以生成可点击的网页链接"
